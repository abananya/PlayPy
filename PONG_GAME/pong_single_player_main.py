import pygame
from PONG import game_ver2_setup
import neat
import os
import pickle
import winsound
...
class PongGame:
    def __init__(self, my_window, width, height):
        self.game = game_ver2_setup.Game(my_window, width, height)
        self.left_paddle = self.game.left_paddle
        self.right_paddle = self.game.right_paddle
        self.ball = self.game.ball
    
    def testing_ai(self, genome, config):
...
def eval_genomes(genomes, config):
    ...
    for i, (genome_id1, genome1) in enumerate(genomes):
        ...
        for genome_id2, genome2 in genomes[i+1: ]:
            ...
            game = PongGame(my_window, width, height)
...
def test_ai(config):
    ...
    with open("best.txt", "rb") as f:
        winner = pickle.load(f)
    game = PongGame(my_window, width, height)
    game.testing_ai(winner, config)
